package algorithm.test.graph.kruskal;
 ...
 * @author j_cong
 * @date 2017年9月14日
 * @describe 邻接表无向图的prim算法生成最小生成树
 * @version V1.0.0
 */
public class ListUDGKruskal {
	 ...
	 */
	public ListUDGKruskal(char[] vexs, EData[] edges) {
	...
	private EData[] getEdges() {
		...
		for (int i = 0; i < mVexs.length; i++) {
			...
			while (node != null) {
				if (node.ivex > i) {
					edges[index++] = new EData(mVexs[i].data,
							mVexs[node.ivex].data, node.weight);
				}
				node = node.nextEdge;
			}
		}
	...
	public static void main(String[] args) {
		...
		// pG = new ListUDG();
		// 采用已有的"图"
		pG = new ListUDGKruskal(vexs, edges);
