package algorithm.test.graph.topological;
 ...
 * @author j_cong
 * @date 2017年9月14日
 * @describe 无回路有向图的拓扑排序，该DAG图通过邻接表实现
 * @version V1.0.0
 */
public class ListDG {
	// 邻接表中表对应的链表的顶点
	...
	public void print() {
		...
		for (int i = 0; i < mVexs.size(); i++) {
			...
			while (node != null) {
				System.out.printf("%d(%c) ", node.ivex,
						mVexs.get(node.ivex).data);
				node = node.nextEdge;
			}
			System.out.printf("\n");
		}
