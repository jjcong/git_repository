package algorithm.test.heap.BinomialHeap;
...
public class BinomialHeap<T extends Comparable<T>> {
	...
	public void insert(T key) {
		...
		if (contains(key) == true) {
			System.out.printf(
					"insert failed: the key(%s) is existed already!\n", key);
			return;
		}
	...
	private void decreaseKey(BinomialNode<T> node, T key) {
		if (key.compareTo(node.key) >= 0 || contains(key) == true) {
			System.out
					.println("decrease failed: the new key("
							+ key
							+ ") is existed already, or is no smaller than current key("
							+ node.key + ")");
			return;
		}
		node.key = key;
	...
	private void increaseKey(BinomialNode<T> node, T key) {
		if (key.compareTo(node.key) <= 0 || contains(key) == true) {
			System.out
					.println("increase failed: the new key("
							+ key
							+ ") is existed already, or is no greater than current key("
							+ node.key + ")");
			return;
		}
		node.key = key;
	 ...
	 * 打印"二项堆"
	 * 
	 * 参数说明： node -- 当前节点 prev -- 当前节点的前一个节点(父节点or兄弟节点) direction --
	 * 1，表示当前节点是一个左孩子; 2，表示当前节点是一个兄弟节点。
	 */
